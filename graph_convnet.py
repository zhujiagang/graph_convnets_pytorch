...
class Graph_ConvNet_LeNet5(nn.Module):
    ...
    def graph_conv_cheby(self, x, cl, L, lmax, Fout, K):
        ...
        if K > 1:
            ...
            x = torch.cat((x, x1.unsqueeze(0)), 0)  # 2 x V x Fin*B ### 2*928*100
        for k in range(2, K):
        ...
        x = x.permute(3, 1, 2, 0).contiguous()  # B x V x Fin x K ### 100*928*1*25
    ...
    def forward(self, x, d, L, lmax):
        ...
        # graph CL1
        x = x.unsqueeze(2)  # B x V x Fin=1 ### 100*928*1
        x = self.graph_conv_cheby(x, self.cl1, L[0], lmax[0], self.CL1_F, self.CL1_K)
        x = F.relu(x) ### 100*928*32
        x = self.graph_max_pool(x, 4) ### 100*232*32

        # graph CL2
        x = self.graph_conv_cheby(x, self.cl2, L[2], lmax[2], self.CL2_F, self.CL2_K)
        x = F.relu(x) ### 100*232*64
        x = self.graph_max_pool(x, 4) ### 100*58*64

        # FC1
        x = x.view(-1, self.FC1Fin) ### 100*3712
        x = self.fc1(x) ### 100*512
        x = F.relu(x)
        x = nn.Dropout(d)(x)
